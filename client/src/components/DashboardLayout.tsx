...
const menuGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { icon: UserPlus, label: "Client Onboarding", path: "/onboarding" },
...
function DashboardLayoutContent({
  ...
  const [openGroups, setOpenGroups] = useState<string[]>(() => {
    const active = menuGroups.find(g => g.items.some(i => i.path === location));
    return active ? [active.title] : [menuGroups[0].title];
  });
        ...
        >
          ...
          <SidebarContent className="gap-0">
            ...
            {menuGroups.map(group => {
              // In icon-collapsed mode there's no room for group headers — show all items.
              const isOpen = isCollapsed || openGroups.includes(group.title);
              return (
                <div key={group.title} className="px-2 py-0.5">
                  <button
                    onClick={() => toggleGroup(group.title)}
                    className="flex items-center justify-between w-full px-3 py-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground/70 hover:text-foreground rounded-md hover:bg-accent/40 transition-colors group-data-[collapsible=icon]:hidden"
                  >
                    <span>{group.title}</span>
                    <ChevronDown className={`h-3.5 w-3.5 transition-transform ${openGroups.includes(group.title) ? "" : "-rotate-90"}`} />
                  </button>
                  {isOpen && (
                    <SidebarMenu>
