...
export const MODEL_COSTS = {
  ...
  "claude-opus-5": { input: 5.0, output: 25.0, name: "Claude Opus 5" },
  "claude-sonnet-5": { input: 2.0, output: 10.0, name: "Claude Sonnet 5" },
  "claude-haiku-4-5": { input: 1.0, output: 5.0, name: "Claude Haiku 4.5" },
...
/**
 * Hard budget enforcement — throws when a client has already reached its monthly budget.
 * Call before starting any paid generation for that client. A client with no budget set
 * (0 / null) is unlimited. Raising the client's monthlyBudget lifts the block.
 */
export async function assertClientWithinBudget(clientId: number): Promise<void> {
  const db = await getDb();
  if (!db) return;
  ...
  if (!client || !client.monthlyBudget) return; // no budget configured = unlimited
